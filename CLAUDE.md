...
| `generate/wallpaper.py` | One Plasma wallpaper package per E16 background image (`WallpaperPackage`); PNG/JPEG/BMP copied through at real dimensions, everything else re-saved as PNG. Two `SET_SOLID` exceptions: alpha-carrying sources with a solid underneath are flattened over it (e13's tanbg.png over black — E16 composites the tile over the solid), and a SET_SOLID-only block (OPENSTEP) becomes a small flat 128×128 package. `pick_default` ranks by `(not solid, area)` — a solid never outranks art. `metadata.json` carries `X-Themey-FillMode` (the six-mode vocabulary above) and, when the block had a SET_SOLID, `X-Themey-SolidColor` (`r,g,b`) for the fit/pad letterbox. Under `--upscale waifu2x` ONLY (read off `ir.Theme.upscale`), a source narrower than `WALLPAPER_UPSCALE_MAX_WIDTH` (1920 — the width at which it already covers the common desktop) is run through the CNN at 2x first, so Plasma DOWNsamples instead of up. Verified 2026-09-02 against LANCZOS-straight-to-1920x1080 on five corpus wallpapers (512×400 … 1280×1024): waifu2x won all five, most visibly on text and fine mechanical detail. Denoise stays at the chrome `-n 0` — `-n 1` kills JPEG mosquito noise on flat dark art but waxes over grain in rust/stone/scanline art, losing 4 of 5. The guard is re-checked against the DOUBLED size, a scaler failure ships the original with a `wallpaper:` note (never a failed convert), and upscaled JPEG sources are re-encoded as JPEG q92 since the byte-for-byte passthrough is forfeit anyway — Aliens' four wallpapers go 707 KB → 5020 KB, dominated by the one PNG source (384 → 4467 KB), which stays PNG on purpose |
...
| root modules | `ir.py` (IR — `Theme.upscale` rides beside `Theme.scale`, the same kind of thing: a render setting, not parsed E16 data. It is on the IR because the generators that need it are reached through chains carrying `Theme` and nothing else — plasmastyle's builders are a fixed `Callable[[Theme], Element | None]` (~35 call sites to thread a parameter) and `wallpaper.write_package` already takes the Theme. It is ALWAYS the EFFECTIVE mode: `pipeline.convert` resolves the waifu2x→hqx fallback BEFORE `build_theme`, so no generator can act on a scaler that will not run), `paths.py` (XDG install roots), `install.py` (atomic deploy — `deploy` for package dirs, `deploy_file` for single files like `.colors`, `clear_style_cache` for the Version-keyed plasmashell kcache, called at both convert and apply time), `report.py` (takes the EFFECTIVE `upscale=` so the Approximated line names the scaler that actually ran — it hardcoded "NEAREST" through every `--upscale quality` run until 2026-09-02, and a waifu2x run that fell back must read hqx), `preview.py`, `kwin.py`, `render.py`, `apply.py`, `external.py` (xcursorgen + waifu2x wrappers), `slug.py` (naming contract), `log.py` |
...
always excluded). Which of the four get built, and how big, is
...
around the pager instead of shrinking every window for it. Only the modes
in `_HIDING_TOKENS` are ever scripted; the rest are file-only and read at
start-up. That table was corrected 2026-09-04 against plasma-workspace
`shell/scripting/panel.cpp` (branch Plasma/6.6): the setter and the getter
are ASYMMETRIC there — `Panel::setHiding` (323-340) matches `autohide`,
`dodgewindows` AND `windowsgobelow` case-insensitively and calls
`setVisibilityMode` straight away, while `Panel::hiding` (303-321) has
cases only for NormalPanel/AutoHide/DodgeWindows and answers `"none"` for
WindowsGoBelow. That is what the earlier live probe of the WindowsGoBelow
spellings actually measured: the write took, the READ-BACK could not name
it, so "the scripting engine has no string for that mode" was wrong.
themey still routes WindowsGoBelow through the file alone — deliberately
UNCHANGED here, pending a live re-check — so the creation/re-assert
scripts omit the `hiding` assignment entirely for such a panel and
...
its strut, 2 for the dock, which IS scripted and so needs no restart to
dodge), dead last of the panel section: it must follow every script
...
`_furniture_specs()` with defaults still returns all four, which is what
...
`org.themey.dock` for the dock; a re-asserted
...
A FOURTH furniture panel, the DOCK (`_DOCK_KEY` = `DockPanel`,
`_dock_spec`, `--dock`/`--no-dock`/`--dock-size`): the one with no E16
ancestor — a floating, centred, bottom-edge, content-sized
(`lengthMode=fit`) panel hosting `org.themey.dock`, dodging windows
(`_VISIBILITY_DODGE`, the one furniture visibility `_HIDING_TOKENS` can
also script). Thickness is `dock_thickness_px(scale)` =
`max(scale_px(32, scale), 48)` — 48/48/64/96 px at scale 1/1.5/2/3, no
E16 source to quote, just twice the dragbar and two thirds of the iconbox
— overridden by `--dock-size`. The creation script turns
`showOnlyCurrentDesktop` OFF (the applet's own default is on, which would
make the dock a second per-desktop iconbox: a dock is the machine-wide
window list where E16's iconbox is the per-desktop minimized one) and
writes `taskHoverEffect` from the style exactly as the iconbox does, both
re-asserted on every apply. `_set_panels_fit` un-floats EVERY panel, so
the dock's own re-assert (`FurnitureSpec.floating`) must run after it and
does — never let that ordering invert, the fork's 1.24 zoom cap assumes a
floating panel. `_dock_spec` is the only spec built OUTSIDE
`_furniture_specs` as well as in it, because `themey dock`
(`apply.apply_dock`, `cli.dock_cmd`) needs this one panel without reading
the other three: it passes `_ensure_furniture(specs=(dock,))` — the ONLY
caller of that parameter — so no other marker is read, re-asserted or
dropped, and it runs NO `plasma-apply-*` tool, no `_set_panels_fit`, no
KWin reconfigure and no plasmashell restart (the dock appears
immediately). Its per-theme settings come off whatever global theme is
active, read once: `_active_themey_bundle` (kdeglobals `[KDE]
LookAndFeelPackage`, only when it is a `themey_*` bundle) feeds
`_active_theme_scale` (that bundle's `X-Themey-Scale`, else
`_DEFAULT_THEME_SCALE`) and `_active_tasks_hover` (its Plasma Style's
`X-Themey-TasksHover`). `_active_tasks_hover` returning None means NOBODY
has an opinion — a stock desktop — and then `taskHoverEffect` is left out
of both the creation and the re-assert script entirely, so the applet's
own `main.xml` default stands and a later run cannot silently undo a
theme-derived value. `themey dock --remove` removes the recorded panel and
clears the marker, raising on failure (removing the dock is the whole
point of that call) where the apply path only warns; a dock created before
the first full apply lands in `PrevPanelLengthModes`/`PrevPanelFloating`
harmlessly, since `revert` removes it BEFORE restoring those. Just before the furniture, when the pager is asked for, `_set_desktop_grid_column` stacks the
...
pager/iconbox/dragbar/dock panels (before the panel-mode restore, so that script iterates only
...
well as what `qmldeco.write` and `write_report` get, so the decoration, the
Plasma Style and the report line can never disagree. The decision needs no
Theme — only the `upscale:` note does, and `notes` is the one mutable field,
so it is appended immediately after construction. `upscale_part` stays
...
`--target dock` themey's dock applet in a bottom-edge panel (two kdialog
clients so the row plates a focused task beside an unfocused one; both go
through the shared `_run_applet_session`, whose `clients` is a COUNT).
`resolve_plasmoid_dir(plugin_id, work)` picks the package for either: the
one the on-the-fly convert just wrote under the work dir, else the
installed copy under `paths.plasmoids()`.
