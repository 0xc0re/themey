...
// A plain Item, not a ToolTipArea: the fork moved the tooltip out to avoid
// the jitter ToolTipArea's own hover handling caused during the zoom.
Item {
    id: taskItem

    // themey re-enables the per-task tooltip (a dock with no task names is
    // a worse dock) — the fork left it inert at `active: false`.
    //
    // It follows `highlighted`, not the ToolTipArea's own containsMouse:
    // the list's hover sensor (TaskList.qml globalMouseArea, z: 1000) sits
    // above every delegate and consumes the hover events, so containsMouse
    // here is not a dependable source. For the same reason `active` alone
    // would not be enough — showToolTip()/hideToolTip() are called
    // explicitly on top of it.
    //
    // The fork's comment blames ToolTipArea's internal hover handling for
    // jitter during the zoom; that is unverified against this arrangement
    // and wants a look on a live desktop. The fallback is a short settle
    // timer on `hovered` before showing.
    PlasmaCore.ToolTipArea {
        id: toolTip
        anchors.fill: parent
        mainText: taskItem.taskName
        subText: taskItem.isLauncher ? i18n("Click to launch") : ""
        icon: taskItem.iconSource
        location: taskItem.panelLocation
        active: taskItem.highlighted && !taskItem.isDragging && !taskItem.isGroupParent
        onActiveChanged: {
            if (active) {
                showToolTip();
            } else {
    ...
    // Hover state. `hoveredByList` is TaskList's own pointer tracking: the
    // dock zooms a whole neighbourhood, and the list's sensor sits above
    // every cell, so the list — not this MouseArea — is the authority on
    // which task is under the pointer. `highlighted` is what the plate, the
    // icon and the tooltip react to, mirroring upstream Task.qml:76.
    property bool hovered: mouseArea.containsMouse
    property bool hoveredByList: false
    readonly property bool highlighted: hovered || hoveredByList

    // Plasma Style art (themey's generate/plasmastyle widgets/tasks.svg).
    // False = the active style has no `center` element and the fork's own
    // Kirigami rectangles are all there is.
    property bool hasTaskArt: false
    // Plasmoid.configuration.taskHoverEffect, which themey apply writes
    // from the style's X-Themey-TasksHover stamp.
    property bool hoverEffect: true
    // An invisible FrameSvgItem on the `normal` prefix, owned by TaskList:
    // its fixedMargins are the plate's own bevel, which the icon is inset
    ...
    readonly property real frameInset: (hasTaskArt && metricFrame)
        ...
        : 0
    // With a plate behind it the icon is framed by real art and can fill
    // the inset cell; without one it keeps the fork's 6%-a-side padding
    // against clipping.
    readonly property real iconFit: hasTaskArt ? 1.0 : 0.88
    ...
    Item {
        ...
        Kirigami.Icon {
            ...
            // Without theme art the fit is the fork's 0.88 — ~6% padding on
            // each side against clipping for icons that have none of their
            // own (PNGs, some SVGs). With art the plate supplies the frame,
            // so the icon fills the cell inset by the plate's own bevel.
            width: Math.round((parent.width - taskItem.frameInset)
                              * taskItem.iconSizePercent / 100 * taskItem.iconFit)
            height: Math.round((parent.height - taskItem.frameInset)
                               * taskItem.iconSizePercent / 100 * taskItem.iconFit)
            source: taskItem.iconSource
            fallback: "application-x-executable"
            active: taskItem.highlighted
        }
    ...
    // The Plasma Style's own task plate — themey synthesizes widgets/tasks
    // from the E16 iconbox button art (generate/plasmastyle), so this is
    // where the theme actually shows up in the dock.
    //
    // A SIBLING of iconContainer, not a child: inside that item's
    // `layer.enabled` FBO the plate would be rasterised once at the
    // container's ~0.78 rest scale and stay soft; pinned to the static cell
    // instead it would read as a hole the icon lifts out of. Following
    // iconContainer's own centre offsets and size, it stays crisp 1:1 at
    // rest and lifts and grows WITH the icon.
    KSvg.FrameSvgItem {
        id: plate
        z: 0
        visible: taskItem.hasTaskArt
        ...
        readonly property real extras: iconContainer.animatedPressScale
            * iconContainer.attentionScale * iconContainer.dropScale
        width: Math.round(taskItem.baseSize * taskItem.displayZoom * extras)
        height: width

        // Upstream Task.qml:470-472 verbatim, with the fork's own
        // panelLocation and hoverEffect standing in for Plasmoid.*.
        property bool isHovered: taskItem.highlighted && taskItem.hoverEffect
        property string basePrefix: "normal"
        prefix: isHovered
            ? TaskTools.taskPrefixHovered(basePrefix, taskItem.panelLocation)
            : TaskTools.taskPrefix(basePrefix, taskItem.panelLocation)
    }

    // basePrefix precedence, upstream Task.qml:640-673: the first state
    // whose `when` holds wins, so launcher beats attention beats minimized
    // beats focus, and the plate's own "normal" is the fallthrough.
    states: [
        State {
            name: "launcher"
            when: taskItem.isLauncher
            PropertyChanges { plate.basePrefix: "" }
        },
        State {
            name: "attention"
            when: taskItem.isDemandingAttention
            PropertyChanges { plate.basePrefix: "attention" }
        },
        State {
            name: "minimized"
            when: taskItem.isMinimized
            PropertyChanges { plate.basePrefix: "minimized" }
        },
        State {
            name: "active"
            when: taskItem.isActive
            PropertyChanges { plate.basePrefix: "focus" }
        }
    ]
    ...
    Rectangle {
        ...
        height: width
        radius: width / 2
        // With theme art the focus plate already carries the accent bar
        // generate/plasmastyle paints on the panel-adjacent edge, so a
        // second active marker is noise.
        visible: !taskItem.isLauncher && !taskItem.isGroupParent
                 && !(taskItem.hasTaskArt && taskItem.isActive)
