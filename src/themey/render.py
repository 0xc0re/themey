...
def resolve_theme_dir(theme: str, *, scale: int, work: Path) -> tuple[str, Path]:
    ...
    if p.suffix == ".etheme" or p.is_file():
        ...
        result = convert(p, scale=scale, output_dir=work / "convert")
...
def render(
    ...
    *,
    ...
    try:
        name, theme_dir = resolve_theme_dir(theme, scale=scale, work=work)
        cfg = work / "config"
        data = work / "data"
