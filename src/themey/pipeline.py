...
def convert(etheme_path: Path, *, scale: int = 2) -> ConvertResult:
    ...
    with extract(etheme_path) as raw:
        ...
        # (same filesystem as the install target — required for os.replace).
        with tempfile.TemporaryDirectory(
            ...
            # Atomic install: renames stage_theme_dir into final position.
            # After this call, stage_theme_dir no longer exists inside the
            # TemporaryDirectory — the TemporaryDirectory cleanup is a no-op
            # for that subdir.
        ...
        # These run AFTER the `with extract(...)` block closes because they
        # only need Theme IR data (theme.notes, theme.palette, etc.) — NOT
        # theme.asset_root. See lifecycle invariant in module docstring.
